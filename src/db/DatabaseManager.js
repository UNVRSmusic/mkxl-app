...
class DatabaseManager {
    ...
    `);
        ...
        this.db.run(`CREATE INDEX IF NOT EXISTS idx_patches_category ON patches(category)`);
        this.db.run(`CREATE INDEX IF NOT EXISTS idx_patches_favorite ON patches(favorite)`);
        this.db.run(`CREATE INDEX IF NOT EXISTS idx_patches_rating ON patches(rating)`);
    ...
    `);
        if (!results[0])
            return [];
        return results[0].values.map((row) => this.rowToPatch(results[0].columns, row));
    }
    /**
     * Get patch by ID
     */
    ...
    savePatch(patch) {
      ...
      `, [
            ...
            const patchId = results[0].values[0][0];
            // Add tags
            if (patch.tags) {
    ...
    getOrCreateTag(tagName) {
        ...
        return newResults[0].values[0][0];
    }
    /**
     * Update tags for a patch
     */
    updatePatchTags(patchId, tags) {
        if (!this.db)
            return;
